...
compile_models <- function(comparison) {
  ...
  compile_chains <- function(comparison) {
    ...
    sets <- base::split(tasks,  list(tasks$comparison,tasks$model,tasks$growth_measure, 
                                     tasks$rho_combo, tasks$gap_combo, tasks$size_combo, 
                                     tasks$kfold), sep='_', drop=TRUE)
    pars <- lapply(sets,  function(s) s[1, c("comparison", "model", "growth_measure", 
                                             "rho_combo", "gap_combo", "size_combo", "kfold")])

...
plot_species_curves <- function(model, data, growth_range= c(0.03,0.5), hazard_curve = FALSE) {
  preds <- predict_species(model, data, growth_range)
  if(hazard_curve ==FALSE) {
    p1 <- ggplot(preds, aes(x = dbh_growth,y = annual_prob_mort, group = sp, colour = wood_density)) + 
...
plot_wd_effects <- function(model,wood_density=c(0.3,0.8), growth_range = c(0.03,0.5), hazard_curve = FALSE) {
  preds <- predict_wd_effect(model,wood_density, growth_range, hazard_curve)
...
plot_fig3 <- function(model, data) {
  p1 <- plot_wd_effects(model, hazard_curve= TRUE) + ggtitle('Median species')
  p2 <- plot_species_curves(model, data, hazard_curve = TRUE) + partial_plot_theme(legend.position= c(0.8,0.5)) + ggtitle('All species')
  p3 <- plot_wd_effects(model, hazard_curve= FALSE)
  p4 <- plot_species_curves(model, data, hazard_curve = FALSE) + partial_plot_theme()

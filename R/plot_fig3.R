...
plot_fig3 <- function(spp_params, mu_trend) {
  ...
  trait_label_lookup <- c(wood_density ="Wood~density~(g~cm^-3)", gap_index = "Shade~intolerance", dbh_95 = "Max~dbh~(cm)")
  param_label_lookup <- c(alpha = "Low~growth~effect~(alpha)", beta = "Growth~decay~rate~(beta)", gamma ="Baseline~hazard~(gamma)")
  
  ggplot2::ggplot(spp_params, ggplot2::aes(x = trait_value,y = mean)) + 
    ggplot2::geom_ribbon(data = mu_trend, ggplot2::aes(x = trait_value,ymin = `2.5%`,ymax = `97.5%`), alpha=0.5, colour=NA) +
    ...
    ggplot2::geom_line(data = mu_trend, ggplot2::aes(x = trait_value, y= mean), col='blue') +

get_chunks_model1_constant <- function() {
  ...
  for (i in 1:n_obs) {
  p[i] <- inv_cloglog(log(census_length[i] * (exp(c_log))));
  }",
  model = "",
...
get_chunks_model2_constant <- function() {
  ...
  for (i in 1:n_obs) {
  p[i] <- inv_cloglog(log(census_length[i] * (exp(a_log - exp(b_log) * growth_dt_s[i]))));
  }",
  model = "",
  r_model = function(stan_data, pars) {
    a_log <- pars$a0
...
get_chunks_model3_constant <- function() {
  ...
  for (i in 1:n_obs) {
  p[i] <- inv_cloglog(log(census_length[i] * (exp(a_log - exp(b_log) * growth_dt_s[i]) + exp(c_log))));
  }",
  model = "",
  r_model = function(stan_data, pars) {
    a_log <- pars$a0

...
More recently, researchers have used plant traits in an attempt to explain inter-specific variability in mortality rates. For example, ....
However, to date, most of these studies have assumed that these trait effects are constant through time and do not interact with changing environmental conditions or individual state variables such as growth rate or size.

###Aims
Based on these knowledge gaps I have three primary objectives:
...
3) Build a process-based mortality model that incorporating the common hazard experienced by plants and how traits mediate their effects. 
...
bci.mainstem<- bci.mainstem %>%
  ...
  ln_census = scale(log(bci.mainstem$census.interval))[,1]/2, #logged for model
...
cat('
  data {
	...
	model {
		for (it in 1:n_obs)
			died[it] ~ bernoulli(inv_cloglog(ln_census[it] + log(c0 * exp(-c1 * wd[it]) + c2 * exp(-c3 * dbh_gr[it]))));
	}

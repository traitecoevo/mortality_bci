...
\authorcontributions{J.C. designed and performed the analysis; D.F. conceived the idea, supervized the development of this work; S.J.W \& M.W. provided biological insight. All authors contributed to the writing of this manuscript.}
...
\begin{abstract}
Death is an integral process driving population dynamics, nutrient cycling, and evolution within plant communities. While past research has identified factors influencing tree mortality -- including an individual's past growth, species-level functional traits, and temporal plot-scale effects; these are yet to be integrated within a general predictive framework. Here we estimate instantenous mortality rates of 203 tropical rainforest tree species at Barro Colorado Island (BCI), Panama, by combining classic survival analysis techniques with a cross-validated hierarchical Bayesian framework that integrates individual-level, species-level, and plot-level effects. We show that greatest predictive capacity is achieved when when models simultaneously estimate growth-dependent and growth-independent hazards, use stem diameter growth rather than basal-area growth and account for wood density effects on growth-independent terms. Moreover, we show that the effects of wood density -- a prominent trait known to influence mortality -- capture only a portion of total species-level differences; and that the year-to-year variation in mortality was large.  Our analysis also suggests that growth-dependent processes -- and thus competition -- predominate as the largest driver of mortality at BCI. Combined, this study provides a framework for predicting individual-level mortality in highly diverse tropical forests, but also highlights how little we now about the causes of species-level and temporal plot-scale effects, needed to effectively predict tree mortality.
\end{abstract}
...
\caption{Outline of methodology. A) Our data consist of repeat measures on stem diameter ($D$) and status ($S$, alive or dead) for individual tress at specific census dates ($t_1$, $t_2$, $t_3$). B) We consider three alternative mortality functions, combining growth-independent and growth-dependent effects. For each model form, we consider two alternative predictors of growth, $X$ (basal area and stem diameter growth), as well as allowing for species-level effects on the parameters $\alpha$, $\beta$ and $\gamma$. C) The predictive skill of alternative models is evaluated via 10-fold cross validation. The entire dataset is split into 10 folds ($F_1$, ..., $F_{10}$). We then refit each alternative model 10 times ($M_1$, ..., $M_{10}$), using different combinations of testing and training data. We then average the log loss $\mathcal{L_i}$ for each of these 10 model fits, to give an overall average log loss for each model $\bar{\mathcal{L}}$.}
\label{fig1}
\end{figure}

In this study, we reconcile both intraspecific and interspecific mortality theory by combining classic survival analysis techniques with a cross-validated hierarchical Bayesian framework. Specifically, our framework  instantaneous mortality rates for 203 tropical rainforest trees at Barro Colorado Island (BCI) by integrating individual-level, species-level, and plot-level effects and simultaneously estimating growth-dependent and growth-independent mortality processes. Using this framework we 
identify the most parsimonious model of tropical tree instantaneous mortality rates. We also ask: 1) which  growth measure (i.e. diameter increment growth or area growth)best predicts growth-dependent mortality; 2) Does including wood density improve model predictive capacity, and if so, does it predominately affect baseline hazard, growth-dependent hazard or both? 3) What is the proportion of species variability explained by wood density?


\section*{Framework for modelling mortality}

In classic survival analysis, the probability that an individual dies between $t_2$ and $t_3$, $p_{t_2\rightarrow t_3}$ is defined by
\begin{equation} \label{eq:mortality_model}
p_{i,t_2\rightarrow t_3} = 1 - \exp\left(- \int_{t_2}^{t_3} \lambda_i(t) {\rm d}t\right),
\end{equation}
where $\lambda_i(t)$ is the instantaneous mortality rate of death per unit time \cite{Kleinbaum:2012tn} and $\int_{t_2}^{t_3} \lambda_i(t) {\rm d}t$ is the cumulative hazard between $t_2$ and $t_3$. The challenge then is to identify suitable forms of the instantaneous hazard (mortality) rate function.

We examine the performance of three functional forms of $\lambda_i(t)$ to predict observed patterns of tropical tree mortality:
\begin{align}
...
\caption{Average logarithmic loss (lower = higher accuracy). A) Four sequential stages of model selection. Null model: shows the expected logloss for the null model with no census effect; because mortality varied substantially between censuses it was included on all subsequent models; Functional form: comparison between baseline hazard model ($\gamma c$; circle), growth\textendash dependent hazard model ($(\alpha e^{\beta X_{i}}) c$; triangle) and model incorporating both baseline and growth\textendash dependent hazards ($(\alpha e^{\beta X_{i}} + \gamma) c$; square); Wood density: Most parsimonious effect of wood density; Species effects: Model with species random effects on all parameters \textendash indicating the total gain in predictive capacity if accounting for all species variability. B) Wood density effects on all parameter combinations in the model that incorporates both baseline and growth-dependent hazards. Colors represent growth measure used: white = no growth measure, grey = basal area growth, black = dbh growth. Errors are $\pm$ 95\% BCI.}
\label{fig2}
\end{figure}
...
\caption{Estimated relationship between wood density and baseline mortality rate. Points are species estimates and solid line is the mean species trendline. All errors are $\pm$ 95\% BCI.}
\label{fig3}
\end{figure}
...
\caption{Predicted instantaneous mortality rates its derivative annual mortality probability. A,C) Effect of high (0.8) and low (0.3) wood density on growth-mortality relationship. B,D) Predicted growth\textendash mortality response curves for each species. Errors are $\pm$ 95\% BCI.}
\label{fig4}
\end{figure}

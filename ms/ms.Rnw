...
\subsection*{Most predictive functional form}

Full model most predictive (also most biologically reasonable)
DBH DT most predictive given functional form

\subsection*{What is the most predictive combination of parameter-wood density effects?}

wood density on gamma. Technically significant on alpha too... but effect size is small (can't show this though as focus is on predictive capacity with cross val.)

\subsection*{How well does our model predict if we allow full random effects?}

Seems to account for ~20-30\% of species variability (this will be confirmed once other models are finished).

Figures:
Single figure of logloss
figure of species gamma values vs rho
response curve for low and high wood density species from dataset.

Species variability is highest on gamma (1.2) followed by alpha (0.8) and least on beta (0.3)
Mention proportion of species variability on gamma explained by wood density.

...
\caption{Proportional reduction in predicted 1-year survival from removing different effects. Effects are not mutually exclusive so sum to more than 1.0. Note also that the effect of "wood density" is nested within the effect of "species".}  
\label{fig5}
\end{figure}

...
\end{equation}
For models that did not contain species random effects, the above sigma's were not calculated and only the exponentiated means (i.e. $\mu_{\log \alpha_0}$,$\mu_{\log \beta}$,$\mu_{\log \gamma_0}$) were used.

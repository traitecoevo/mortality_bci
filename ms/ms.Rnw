...
The initial dbh, $\widehat{dbh}_{i,t-1}$, was sampled as a random realisations from a lognormal distribution:
\begin{equation} \label{eq:true_dbh1}
\widehat{dbh}_{i,t-1} \sim \mathcal{LN}(\mu_{dbh_{i,t-1}}, \sigma_{dbh_{t-1}}), 
\end{equation}
...
The final dbh measurement, $\widehat{dbh}_{i,t}$ was estimated as:
\begin{equation}\label{eq:true_dbh2}
\widehat{dbh}_{i,t} = \widehat{dbh}_{i,t-1} + R_i \, T_i, 
\end{equation}
where, $R_i$ represents the annual true increase in dbh (cm) and $T_i$ represents the number of days between measurements divided by 365.25. We modelled $R_i$ as a random realisation from a lognormal distribution centered on zero and a standard deviation of 1. This prior conveyed our expectation that annual increases in dbh were unlikely to be greater than 7 cm/yr. Sampling from a lognormal distribution also ensured growth rates could not be negative. 

Using this model we then used \stan's \textit{optimizing} function to maximise the joint posterior and extract point estimates of true initial dbh, $\widehat{dbh}_{i,t-1}$, true final dbh, $\widehat{dbh}_{i,t}$, and true annual dbh growth $R_i$. We used annual diameter growth, $R_i$, and annual basal area growth:$\frac{(\pi \, 0.25 \,\widehat{dbh}_{i,t}) - (\pi \, 0.25 \, \widehat{dbh}_{i,t-1})}{T_i}$ in the subsequent mortality analysis.
...
p_{t_1\rightarrow t_2} = 1 - \exp\left((t_2-t_1)\,\lambda_i\right)
\end{equation}
where, $\lambda$, is the instantaneous hazard rate of death per unit time \citep{Kleinbaum:2012tn}. 

Here we examine the performance of three functional forms of $\lambda$ to predict observed patterns of tropical tree mortality. The simplest model (eqn: \ref{base_haz}), assumes a single baseline hazard, which is constant irrespective of a plants growth rate. The second model (eqn: \ref{growth_haz} describes a growth-dependent hazard which assumes plants can only die due to a growth related cause. In this model, we make an explicit assumption that the growth-mortality relationship is captured by a negative exponential curve whereby the probability of death is high at low growth rates and as growth increases the probability of dying decays at an exponential rate until an asymptote at zero is reached. This negative exponential relationship has been used in other studies to model growth-dependent mortality \citep[e.g.][]{Kobe:1995tw} and broadly reflects patterns observed in non-parametric growth-mortality models \citep{Wyckoff:2000un, Kunstler:2005bn, Vieilledent:2010fv}. The third model (eqn: \ref{add_haz}, and potentially the most biologically realistic, is the summation of the two previous models. Specifically, this model allows $\lambda$ to decrease exponentially with increasing growth, but, unlike a standard negative exponential, will asymptote at baseline hazard instead of zero. The advantage of this model is that it effectively accounts for two independent causes of mortality: that due to growth and that due to chance (i.e. stochastic events such as windthrow and fire). The parameters of eqn: \ref{add_haz} are also biologically interpretable: $\alpha$ represents the intercept (i.e. sum of both growth dependent and growth independent mortality at zero growth rate) and biologically defines the instantaneous mortality hazard $\lambda$ at zero growth. $\beta$ represents the exponential decay rate or the sensitivity of hazard rates to change with growth rate; and $\gamma$, is the asymptote and defines the baseline hazard. $\alpha$ and $\beta$ are intrinsically linked to growth rate, and as such are growth-dependent parameters. By contrast, $\gamma$ is a growth-independent parameter that accounts for stochastic events (e.g. windfall, fire) that can kill a plant irrespective of its growth rate.
\begin{align}
\lambda &= \overbrace{\gamma}^{\text{baseline hazard}} \label{base_haz}\\
\lambda &= \overbrace{\alpha \, \exp\left(-\beta \, Growth_i \right)}^{\text{Growth-dependent hazard}} \label{growth_haz}\\
\lambda &= \overbrace{\alpha \, \exp\left(-\beta \, Growth_i \right)}^{\text{Growth-dependent hazard}} + 
\overbrace{\gamma}^{\text{Baseline hazard}} \label{add_haz}
\end{align}

A central aim of this study is to identify what is the most predictive parameterisation of $\lambda$. To do this, we used K-fold cross-validation and four iterative stages of model fitting and comparison. Stage 1 involved comparing predictive performance of the three $\lambda$ models described above (eqn: \ref{base_haz,growth_haz,add_haz}). In this stage each model was fitted with fixed parameters (i.e. parameters were constant across species) and then again by allowing parameters to varying by species. This allowed us to determine: 1) whether predictive performance was substantially improved with increasing functional complexity; and 2) the extent to which species hazard rates differ. In stage 2 we used the most predictive model from stage 1 to examine whether dbh growth or basal area growth was a better predictor of plant mortality. In stage 3 we used the best model from stage 2, but without species random effects, to determine what combination of parameter-wood density effects best predicted tree mortality. This stage also allowed us to determine whether wood density primarily affected growth-dependent or growth-independent (i.e. baseline) mortality. Finally, in stage 4, we fit the best model based on the previous three steps to the entire dataset with species random effects on all parameters. We then examined wood density effect sizes and the proportion of species variability that it explained. 

\subsection{Model fitting}

Our observations are of survival status $y_i$ (0 = alive, 1 = died) for each individual $i$ between two censuses. We model these observations as random realizations from a Bernoulli distribution based on the probability that individual $i$ dies between $t_1$ and $t_2$, $p_{i,t_1\rightarrow t_2}$:
\begin{equation}
y_{i,t_1\rightarrow t_2} \sim \textrm{Bernoulli}(p_{i,t_1\rightarrow t_2}).
\end{equation}

We modelled $p_{i,t_1\rightarrow t_2}$ using a complementary log-log link as:
\begin{equation}
\log(-\log(1 - p_{i,t_1\rightarrow t_2})) = \log((t_2-t_1)\,\lambda_i)
\end{equation}.
This parameterisation allowed for additive hazards such as those in \ref{add_haz} and is equivilant to a competiting risks analysis \citep{Kleinbaum:2012tn}.

As we have multiple hypothesized functional forms of $\lambda$ (e.g. eqn: X ), each containing parameters that may or may not vary by species, we outline the fitting procedure of our most complicated model of which all others are nested within:
\begin{equation}
\lambda_i = \overbrace{\alpha_{s[i]} \, \exp\left(-\beta_{s[i]} \, Growth_i \right)}^{\text{Growth dependent}} + 
\overbrace{\gamma_{s[i]}}^{\text{Growth independent}}.
\end{equation}  

In this model, $\alpha_s$, $\beta_s$ and $\gamma_s$ varying by species a function of both wood density (measured at species level) and a species random effect:
...
Both $\mu_{\log \alpha_0}$, the average log species residual error at zero growth, and $\mu_{\log \gamma_0}$, the average baseline log species residual error, from normal distributions centered on zero with a standard deviation of 2.5.
\begin{equation}
\mu_{\log \alpha_{0}},\mu_{\log \gamma_0} \sim \mathcal{N}\left(0, 2.5\right).\\
\end{equation}

We estimated $\mu_{\log \beta_0}$, the average species error on the exponential decay rate, using a weak normal prior centered on zero with a standard deviation of 5. 
\begin{equation}
\mu_{\log \beta_0} \sim \mathcal{N}\left(0, 5\right).\\
\end{equation}
As we were fitting these models to multiple growth measures (i.e. dbh increment, basal area growth), we intentially made this prior weak, such that the range of possible values spanned from a rapid exponential decay to no change in mortality with increasing growth.
...
\end{equation}
For models that did not contain species random effects, the above sigma's were not calculated and only the hyperparameter means (i.e. $\mu_{\log \alpha_0}$,$\mu_{\log \beta}$,$\mu_{\log \gamma_0}$) were estimated.

Wood density effects (i.e. $\alpha_1$, $\beta_1$ and $\gamma_1$) were estimated using normal priors centered on zero and a standard deviation of 5:
\begin{equation} \label{eq:rho_effects}
\alpha_1, \beta_1, \gamma_1 \sim \mathcal{N}\left(0, 5\right).\\
\end{equation}

All models were fit using package rstan version 2.8.1 \citep{StanDevelopmentTeam:2015uz}. Three independent chains were executed and in all cases modelled parameters converged within 2000 iterations. Convergence was assessed through both visual inspection of chains and reference to the Brooks-Gelman-Rubin convergence diagnostic \citep{Gelman:1992ts, Brooks:1998ju}. After discarding the first 2000 iterations as ‘burn in’, a further 2000 iterations were taken from the joint posterior.

\subsection{Model evaluation}

We examined predictive accuracy of each model by using K-fold cross-validation. This involved randomly partitioning the data into ten folds of X observations, ensuring that each K-fold had at least one observation per species. We then fitted models to nine folds and predicted to the tenth fold. This was repeated until each fold had been used nine times for model fitting, and once as a heldout dataset for predicting. Cross-validation is a typical approach to model evaluation, and assesses a model’s accuracy when extrapolating to data not used in model calibration, thereby indicating the model’s predictive accuracy (Stone:1997tg).

Predictive accuracy was quantified as the mean (over 10 cross-validation replicates) of each of two threshold-independent performance metrics. First we compare log likelihoods of heldout data, which define the log probability of observing the data given the model. Secondly, in order to examine the model capacity to correctly discriminate between whether an individual survived or not, we used the area under the receiver operating characteristic (ROC) curve (AUC). AUC describes the model’s discrimination capacity as the frequency with which it predicts a higher probability of mortality for a randomly selected observed death than for a randomly selected observed survivor. 
\pagebreak
\bibliography{refs}
\end{document}
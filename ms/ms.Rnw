...
\end{equation}
This parameterisation allowed for additive hazards such as those in eqn:\ref{add_haz} and is equivilant to a competiting risks analysis \citep{Kleinbaum:2012tn}.

As we have multiple hypothesized functional forms of $\lambda$ (e.g. eqn:\ref{base_haz}, \ref{add_haz}), each containing parameters that may or may not vary by species, we outline the fitting procedure of our most complicated model of which all others are nested within:
\begin{equation}

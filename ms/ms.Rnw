...
\authorcontributions{J.C. designed and performed the analysis; D.F. conceived the idea. M.W. supervised the development of this work and provided biological and statistical advice. All authors contributed to the writing of this manuscript.}
...
\caption{three possible functional curves of plant mortality. Estimating growth. Cross validation proceedure}
\label{fig1}
\end{figure*}
...
\caption{Average logarithmic loss $\pm$ 95\% BCIs from 10-fold cross validation. (A) Stage 1: Comparing three functional forms of tropical tree mortality using absolute diameter growth (red) and basal area growth (blue). (B) Comparing model with and without species random effects. (C) Comparing all combinations of wood density effects.}
\label{fig2}
\end{figure}

\subsection*{Predictive performance as a function of model form and growth measure}
We first investigated how model form and growth measure influenced predictive performance. To achieve this we first cross validated three model forms (Fig. \ref{fig1}) of tropical tree mortality using both dbh growth or basal area growth. We found that when the model accounted for both baseline and growth-dependent hazards we obtained a significant improvement in predictive capacity, relative to the other two simpler forms. However, this improvement was dependent on growth measure used, with predictive capacity increasing when dbh growth is used but not when basal area growth is used (Fig. \ref{fig2}). 

\subsection*{Predictive performance and species effects}
Using the most predictive model (i.e. that which used dbh growth and accounted for both baseline and growth-dependent hazards), we then examined how the inclusion of the effect of wood density on various combinations of parameters affected predictive performance. We found that the inclusion of wood density on any of the three parameters significantly improved predictive performance relative to a model without such effects. However, there was little differences in predictive performance across all wood density\textendash parameter combinations. Nonetheless, the most parsimonious model with lowest logarithmic loss was that which contained a wood density effect on only the baseline hazard ($\gamma$). Comparing this model to one that included species random effects on all parameters (i.e. the model that accounts for all species level variation) revealed that wood density accounted for only a small proportion of the maximum predictive gain achievable by accounting for species identification.

\subsection*{Effects of wood denisty}
...
- When examining this interspecific variability we found that the effect of wood density improved model predictive capcity when it was included on the base-line hazard, with only marginal improvements when included on all three parameters.
...
\end{equation}
where, $\lambda$, is the instantaneous hazard rate of death per unit time \cite{Kleinbaum:2012tn} and $T$ is the time lapsed between $t_1$ and $t_2$. 
...
\acknow{
J.C was supported by the Science and Industry Endowment Fund (SIEF; RP04-174). M.W was supported by a fellowship from the Australian Research Council (ARC). D.F. was supported by an ARC discovery grant. We thank Lachlan McCalman and Daniel Steinberg for statistical advice; Anias Gibert etc etc.}

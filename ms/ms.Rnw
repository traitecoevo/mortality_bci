...
In survival analysis, the probability that an individual dies between $t_2$ and $t_3$, $p_{t_2\rightarrow t_3}$ is defined by:
\begin{equation} \label{eq:mortality_model}
p_{i,t_2\rightarrow t_3} = 1 - \exp\left(- \int_{t_2}^{t_3} \lambda_i(t) {\rm d}t\right).
\end{equation}
where, $\lambda$ is the instantaneous hazard rate of death per unit time \cite{Kleinbaum:2012tn} and $\int_{t_2}^{t_3} \lambda_i(t) {\rm d}t$ is the cumulative hazard between $t_2$ and $t_3$.
...
\end{equation}
where $p_{i,t_2\rightarrow t_3}$ is the probability individual $i$ dies in the interval $t_2$--$t_3$. Using standard survival analysis, this probability is given by
\begin{equation}
p_{i,t_2\rightarrow t_3} = 1 - \exp\left(- \int_{t_2}^{t_3} \lambda_i(t) {\rm d}t\right).
\end{equation}
As we only have a point estimate of $\lambda_i(t)$ at $t=t_2$, the cumulative hazard from  $t_2$--$t_3$ is approximated as
\begin{equation}

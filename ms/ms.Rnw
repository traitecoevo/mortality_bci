...
Comparing the three 5-year intervals between censuses from 1995 to 2010, we found average mortality rates progressively decreasing over time. The highest proportion of trees death occurred between 1995\textendash 2000 (\Sexpr{prop_deaths(BCI_model_dataset_true_dbh, 1)}\%) followed by 2000\textendash 2005 (\Sexpr{prop_deaths(BCI_model_dataset_true_dbh, 2)}\%) and 2005\textendash 2010 (\Sexpr{prop_deaths(BCI_model_dataset_true_dbh, 3)}\%). Consequently, when we allowed hazard rates to be scaled by census (i.e. adding term $\delta$) we observed a small, but significant, increase in predictive skill (Fig. \ref{fig2}A).
...
To compliment the above analysis based on skill in predicting held-out data, we used the final fitted model to estimate the amount of variation being contributed by different factors in 1-yr survival for all individuals (Fig. \ref{fig5}). All species-level effects contributed ~\Sexpr{extract_prop_explained(param_variance_explained, I('species'))}\% of predicted variation in 1-yr survival. However, wood density alone captured only \Sexpr{extract_prop_explained(param_variance_explained, I('wood_density'))}\% of total variation. Census effects accounted for an even smaller fraction: only \Sexpr{extract_prop_explained(param_variance_explained, I('census'))}\% of the total predicted variation. Comparing the two independent hazards, growth-dependent hazards (i.e. the part of the model allowed to vary by individual growth) accounted for \Sexpr{extract_prop_explained(param_variance_explained, I('growth_dependent'))}\% of the total variation, while growth-independent hazards accounted for a smaller proportion (Fig. \ref{fig5}).
...
\caption{Proportional reduction in predicted 1-year survival across all individuals from removing different effects. Effects are not mutually exclusive so sum to more than 1.0. Note also that the effect of "wood density" is a subset of the overall "species" effect.}  
\label{fig5}
\end{figure}
...
Future research should also focus on resolving how climate variables influence both growth-dependent and growth-independent hazards, particularly as climate-driven mortality is increasing globally \cite{vanMantgem:2009bi, Mcdowell:2013wf}. Our analyses revealed a marginal decline in mortality at BCI between 1995 and 2010. Unfortunately, we were unable to statistically attribute this temporal change to particular climatic variables due to low temporal replication (N=3). However, the high numbers of deaths that occurred 1995 and 2000, were potentially the product of a severe drought caused by an El-Nino in 1997\textendash 1998 \cite{Condit:1995wl, Mcdowell:2013wf}.

Whether the length between censuses affect the estimation of growth-dependent and growth-independent hazards should also be examined. Large census intervals may lead to underestimation of growth-dependent mortality, and consequently, overestimate the relative contribution of growth-independent hazards. This is because if a census interval is too wide, it will fail to account for deaths attributed to rapid declines in growth caused by senescence (especially for short-lived plants) or events such as drought.

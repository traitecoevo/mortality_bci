...
\begin{abstract}
Individual death is a key process driving population dynamics, nutrient cycling, and evolution within plant communities. While past research has identified factors across a variety of scales influencing tree mortality -- including an individual's past growth, species-level functional traits, and temporal plot-scale effects; these different factors are yet to be integrated within a predictive framework. Here we apply classic techniques from survival analysis within a hierarchical Bayesian and cross-validation framework to derive instantaneous mortality functions for tropical rain forest trees at Barro Colorado Island (BCI) Panama, integrating individual-level, species-level, and plot-level effects. In reaching this point, we show how mortality models including separate growth-dependent and growth-independent components, using stem diameter growth rather than basal-area growth, and attributing the effects of wood density to growth-independent death outperform alternative formulations. Moreover, we show that the effects of wood density -- a prominent trait known to influence mortality -- capture only a portion of total species-level differences; and that the year-to-year variation in mortality was large.  Our analysis also suggests that growth-dependent processes -- and thus competition -- predominate as the biggest driver of mortality at BCI. Combined, this study provides a framework for predicting individual-level mortality in highly diverse tropical forests, but also highlights how little we now about the causes of species-level and temporal plot-scale effects, needed to effectively predict tree mortality.
\end{abstract}
...
\end{equation}
where $\lambda_i(t)$ is the instantaneous hazard (mortality) rate of death per unit time \cite{Kleinbaum:2012tn} and $\int_{t_2}^{t_3} \lambda_i(t) {\rm d}t$ is the cumulative hazard between $t_2$ and $t_3$. The challenge then is to identify suitable forms of the instantaneous hazard (mortality) rate function.
...
Our observations are for survival outcomes $S_{i, t_2\rightarrow t_3}$ (0 = alive, 1 = died) for each individual $i$ between censuses at time $t_2$ and $t_3$. We model these observations as random realizations from a Bernoulli distribution
\begin{equation}

...
Another difficulty when studying mortality -- similar to that faced in other fields like medicine and engineering -- is determining the shape of hazard functions that skilfully predict patterns in mortality. Past research has identified a range of factors correlated with tree survival, including an individual's growth rate \cite{Kobe:1995tw,Wyckoff:2002ul}, and size \cite{Vieilledent:2010fv} as well as species-level traits such as wood density (REFS), maximum size (REFS) and measures of shade tolerance (REFS) \cite{Poorter:2008iu,Kraft:2010kq,Wright:2010fl,AubryKientz:2013dg}. However, these intra- and inter-specific effects have not yet been integrated into a common hazard function that accounts for the two fundamental hazards faced by plants -- death due to low carbon budget and death due to stochastic events that are unrelated to carbon budget (e.g. fire, windthrow) \cite{Hawkes:2000ib,Zens:2003jv,McDowell:2011dr}.

\section*{Towards a unified model of tree mortality}

A specific challenge in developing biologically plausible hazard functions for plants is the need to estimate the independent contributions of carbon-dependent and carbon-independent hazards on an individual's overall hazard. While plants die via many causes, these broadly fall into two main hazards. The first are carbon-dependent hazards, where plants die because of insufficient carbon assimilation for growth and repair. The second are carbon-independent hazards, where plants die because of stochastic events that are unrelated to their growth rate, such as windfall or fire. An individual's total hazard at a given time point is therefore the sum of these two independent hazards, and is contrary to many traditional survival analyses done for plants which assume these hazards are proportional to eachother (REF).
...
Here, we attempt to develop a empirical model capable of reconciling these conflicting growth-mortality relationships observed among individuals and between species while also explicitly estimating the independent contributions of carbon-dependent and independent hazards. We achieve this by evaluating the following, unified hazard function that incorporates individual-, species-, and temporal effects:
...
Comparing the three 5-year intervals between censuses from 1995 to 2010, we found average mortality rates progressively decreasing over time. The highest proportion of trees death occurred between 1995\textendash 2000 (\Sexpr{prop_deaths(BCI_training_full, 1)}\%) followed by 2000\textendash 2005 (\Sexpr{prop_deaths(BCI_training_full, 2)}\%) and 2005\textendash 2010 (\Sexpr{prop_deaths(BCI_training_full, 3)}\%). Consequently, when we allowed hazard rates to be scaled by census (i.e. adding term $\delta$) we observed a small, but significant, increase in predictive skill (Fig. \ref{fig2}A). Individual census effects can be found in Table S1.
...
\caption{Predicted changes in mortality rate (A,B), and 1-yr mortality probability (C,D), with increasing growth rate. A,C) Estimated curves for all 203 BCI species used in this study. B,D) The average ($\pm$ 95\% credible intervals) curve for a high (0.8) and low (0.3) wood density species.}
\label{fig4}
\end{figure}

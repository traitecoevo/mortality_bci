...
% 120 words
\significancestatement{We build a mortality model that not only integrates intra and inter-specific variation but also, simulataneously estimates the two fundamental hazards faced by plants -- Death due to low carbon budget and death independent of carbon budget -- and how a species' light demand, maximum dbh and wood density affect each. We parameterise this model using over 400,000 observed survival records collected over a 15-year period from more than 180,000 individuals across 203 species. We show that carbon-dependent hazards are a significant contribtor to tree mortality at Barro Colorado Island. Moreover, we show that species traits -- wood density, maximum dbh and light demand -- all affect tree mortality rates but ultimately only account for a small fraction of the total variability among species.}
...
B) We consider three alternative hazard functions: 1) a carbon-independent baseline hazard (i.e. average hazard), 2) a carbon-dependent hazard, where growth rate is a proxy for carbon budget; and 3) a function that combines both carbon-dependent and independent hazards. The parameters of the models are biologically interpretable: $\alpha$ defines the low growth rate (i.e. low carbon budget) effect on the overall hazard; $\beta$ reflects the sensitivity of mortality rate to changes in growth rate; and $\gamma$ is the asymptote (Panel B; right). In the most complex model (Panel B; right), $\alpha$ and $\beta$ capture carbon-dependent hazards, while $\gamma$ captures carbon-independent hazards that kill a plant, irrespective of its growth rate/carbon budget (e.g. windfall, fire). For each model form, we consider two alternative predictors of growth, $X$ (basal area and stem diameter growth), as well as allowing for species-level effects on the parameters $\alpha$, $\beta$ and $\gamma$.
...
\end{equation}
Eq. \ref{eq:model} allows for the additive, thus independent, effects of carbon-dependent and carbon-independent hazards, includes a negative exponential effect of individual's growth rate (our measure of carbon budget), and allows for mortality to vary among censuses, via the random effect $\delta_{t}$. Further, the parameters $\alpha_s$, $\beta_s$ and $\gamma_s$ vary by species, $s$. Here we include the effects of three species traits thought to be major drivers of inter-specific variation in tropical tree mortality -- wood density ($\rho$), light rdemand ($\upsilon$) and maximum dbh ($\psi$). We also include species random effects in order to capture species-level differences that are not otherwise accounted for by species traits (for details see Methods).

\begin{figure*}[!ht]
\includegraphics{figures/fig2.pdf}
\caption{Predictive skill of alternative hazard functions. Values are mean ($\pm$ 95\% confidence intervals) logarithmic loss, with lower values implying greater predictive skill and a logloss $\approx$ 0.693 indicating a model unable to distinguish an observed death from a survival.
A) Six sequential stages of model selection with each stage increasing in complexity. \textbf{Stage 1:} Baseline: constant ($\gamma$) vs Baseline with census effect ($\gamma\delta_t$); \textbf{Stage 2:} Best model of stage 1 compared to hazard functions that also account for individual growth rate (i.e. carbon budget). This includes two possible hazard functions: a carbon-dependent only hazard ($(\alpha e^{-\beta X_{i}})\delta_t$) and a baseline + carbon-dependent hazard with census effects ($(\alpha e^{-\beta X_{i}} + \gamma)\delta_t$); \textbf{Stage 3:} The effect of a species' maximum dbh ($\psi$), wood density ($\rho$) and light demand ($\upsilon$) on $\alpha$, $\beta$ and $\gamma$ of most predictive model from stage 2; \textbf{Stage 4:} 2-trait combinations; \textbf{Stage 5:} three-trait combination; \textbf{Stage 6:} Maximum predictive performance by accounting for all species differences via the inclusion of species random effects, $s$ on all three parameters. Symbols represent the three hazard functional form: baseline (circle), carbon-dependent (triangle), and hazard containing both carbon-independent and carbon-dependent hazards (square).}
\label{fig2}
\end{figure*}

To validate this model, we also fit two alternative hazard functions: 1) a baseline hazard (i.e. a null model) and 2) a carbon-dependent only hazard. We then compare each functions ability to predict the survival status of \Sexpr{n_inds(BCI_training_full)} individual trees from \Sexpr{n_spp(BCI_training_full)} species recorded over three 5-year census periods at Barro Colorado Island (BCI), Panama (Fig. \ref{fig1}). The data are repeat censuses of stem diameter and tree status (alive vs. dead) taken approximately at 5-year intervals over a 15 yr period (Fig. \ref{fig1}A). In total \Sexpr{n_obs(BCI_training_full)} observations were used to fit these models. We compare the skill of different hazard functions in predicting outcomes in novel data (i.e. not used in model fitting) via 10-fold cross-validation (Fig. \ref{fig1}D) \cite{Hooten:2015, Stone:1997tg}. Evaluating models in this way is computationally expensive, making it impossible to run all possible model formulations. We therefore fit models across six iterative stages of model development. 

In stage 1, we first ask whether average mortality rates vary substantially among censuses, and thus, whether a census effect is required. We achieved this by fitting a constant, $\gamma$, that is invariant with respect to species, growth rate, or census period (i.e. a null model) and compared it to a constant that is scaled by a census random effect, $\gamma\delta_{t}$. This stage revealed that there was significant variation in mortality rates observed among censuses, and as such a random effect for census was included as a scaling factor for all subsequent models. 

In stage 2, we assessed the predictive performance of three alternative hazard functions (Fig. \ref{fig1}B). The simplest form (Fig. \ref{fig1}B, left) assumes a constant hazard. The second form (Fig. \ref{fig1}B, middle) assumes the risk of dying declines towards an asymptote of zero as growth, and thus carbon budget, increases. The third form (Fig. \ref{fig1}B, right) is the summation of the two previous models; this allows $\lambda_i(t)$ to decrease exponentially with increasing growth rate, but, unlike the second functional form examined here, asymptotes at some carbon-independent baseline hazard $\textgreater 0$. Combined, these three models capture a variety of functional responses that are both used in empirical studies of plant mortality \cite{Kobe:1995tw,Wyckoff:2000un} and are represented in current vegetation models \cite{Moorcroft:2001ws,McDowell:2011dr}, but which have not previously been systematically compared. In doing this, we also investigate which growth measure (stem diameter, stem area increment) more skilfully predicts carbon-dependent mortality. 

In the remaining stages, we take the most predictive hazard function and growth rate measure from stage 2 and examine how species traits: wood density, light demand and maximum dbh, affected model predictive performance. Specifically, we examined each trait individually on all three $\alpha$, $\beta$ and $\gamma$ (stage 3), as well as 2-trait (stage 4) and 3-trait (stage 5) combinations on these three parameters. Lastly, we fit a model that included a species random effect on each parameter (stage 6). This allowed us to 1) examine the maximum predictive capacity when accounting for species differences; and 2) provides a relative indication on how much of the total species variation is captured by these three traits. Because these models were computationally expensive to run (with some models requiring over 400 computing hours), we were unable to examine interactions among traits or determine the most predictive combination of traits effects and parameters in a cross validated framework.

\section*{Results}

\subsection*{Mortality over time}
Examining the three 5-year intervals between censuses from 1995 to 2010, we found average mortality rates declining over time. The highest proportion of tree death occurred between 1995\textendash 2000 (\Sexpr{prop_deaths(BCI_training_full, 1)}\%) followed by 2000\textendash 2005 (\Sexpr{prop_deaths(BCI_training_full, 2)}\%) and 2005\textendash 2010 (\Sexpr{prop_deaths(BCI_training_full, 3)}\%). Consequently, when we allowed hazard rates to be scaled by census (i.e. adding the term $\delta_t$) we observed a small increase in predictive skill (Fig. \ref{fig2}A). Table S1 gives individual census effects.

\subsection*{Hazard functions}
Comparing the three hazard functions summarised in Fig. \ref{fig1}B, we found that the third function --- the function containing both carbon-dependent and carbon-independent hazard terms, i.e. $(\alpha e^{-\beta X_{i}} + \gamma)\delta_t$ --- significantly outperformed both the baseline and carbon-dependent only hazard functions (Fig. \ref{fig2}). Moreover, we found that predictive skill was higher when using stem diameter growth relative to stem-area growth (Fig. \ref{fig2}A). Table S1 gives a summary of mean hyper parameter estimates.

\begin{figure*}[ht!]
\includegraphics{figures/fig3.pdf}
\caption{Estimated relationships between between species traits (maximum dbh, light demand and wood density) and model parameters $\alpha$ (low growth effect), $\beta$ (growth-mortality decay rate) and $\gamma$ (baseline/carbon-independent hazard). Points are the estimated mean ($\pm$ 95\% credible intervals) parameters for each of the 203 Barro Colorado Island species used in this study. Blue trendline with grey shading shows average ($\pm$ 95\% credible intervals) expected relationship across observed trait ranges.}
\label{fig3}
\end{figure*}

\subsection*{Species traits}
Using the hazard function form with highest predictive skill (i.e. $(\alpha e^{-\beta X_{i}} + \gamma)\delta_t$; Fig.~\ref{fig1}b right), we examined how effects of a species' maximum dbh ($\psi$), light demand ($\upsilon$) and wood density ($\rho$) placed on $\alpha$, $\beta$ and $\gamma$ affected model predictive accuracy. In general we found that including trait effects on these parameters decreased logarithmic loss, and thus, increased predictive accuracy (Fig.~\ref{fig2}). Moreover, we found that incorporating the effect of all three traits on these parameters resulted in higher predictive capacity relative to models that only incorporated one or two of the traits. However, this increase in predictive accuracy was small relative to including species random effects on each parameter, which account for unexplained species differences, signifying that substantial inter-specific variation in mortality rates remains unexplained (Fig. \ref{fig2} Stage 6). See Figs. S3-S5 for species-level parameter estimates. 

\begin{figure}[h!]
\includegraphics{figures/fig4.pdf}
\caption{Predicted changes in annual mortality probability with increasing growth rate. A) Estimated average curves for all 203 Barro Colorado Island species used in this study. B,C,D) represent average ($\pm$ 95\% credible intervals) curve for a high (red) and low (blue) trait values. B) wood density values 0.2 vs 0.8 (g cm$^{-3}$); C) maximum dbh values 3 vs 180 cm; D) light demand values 0.5 vs 0.8. Note that the intercept is the sum of $\alpha$ and $\gamma$ (i.e. the stochastic chance of death plus effect of low growth.}
\label{fig4}
\end{figure}

In order to examine how species traits -- maximum dbh, light demand and wood density -- affect tree mortality rates, we fitted a final model containing all three species traits and a species random effect (which captures inter-specific variation that unexplained by traits) on each of $\alpha$, $\beta$ and $\gamma$. We found that each trait influenced tree mortality in different ways (Fig. \ref{fig3}-\ref{fig4}). Specifically, we found that light demanding species were more susceptible to low growth rates (i.e. higher $\alpha$), had higher growth-independent hazards (i.e. higher $\gamma$) and exhibited shallower declines in growth-dependent hazards with increasing growth rates (i.e. lower $\beta$). By contrast, we found that wood density had significant impacts on $\alpha$ and $\gamma$, such that species with high wood density were less susceptible to low growth rates and had lower carbon-independent hazards. Species' maximum dbh only had significant effects on $\gamma$, whereby species that can achieve larger dbh exhibited lower carbon-independent hazards. Table S1 for trait effects on each parameter.
\subsection*{Parameter relative importance}

Using the final model, we then examine: 1) its ability distinguish between individuals that died and individuals that survived via AUROC; and 2) the amount of inter-specific variation the model explains (i.e. $r^2$) relative to the proportions of individual deaths observed per species between 1995 and 2010 (Table.~\ref{table:1}). We found that the full model had an AUCROC of \Sexpr{table1[[1,3]]}, indicating that the model had a fair ability to correctly discriminate observed deaths from survivals. Moreover, we found that the full model also explained \Sexpr{table1[[1,2]]*100}\% of the total variation observed among species.
...
Hmisc::latex(table1,file = "", booktabs = TRUE, rowname = NULL,  
               ...
               from modelled predictions vs proportion of individual deaths observed between 1995 and 2010 for each species. As such, $r^2$ measures the amount of species-level variation explained by the model or sub-models. By contrast, AUROC examines the model's ability to distinguish individuals that died from individuals that survived.")
@

We then examined the relative contribution species traits, random effects and carbon-dependent and independent hazards (Table.~\ref{table:1}) had on overall hazard rates by individually removing their effects and examining changes in both $r^2$ and AUROC. First, we found that the removal of census random effect $\delta_t$ had little impact on either $r^2$ or AUROC. The removal of either wood density or light demand resulted in similar magnitude effects on both $r^2$ and AUROC. By contrast, the removal of maximum dbh had a slightly smaller effect on both metrics. We detected a significant drop in both $r^2$ and AUROC when species random effects (i.e. the parameters that capture variation that is otherwise unexplained by traits) were removed, further highlighting that maximum dbh, light demand and wood density explain only a small fraction of the overall inter-specific variation observed among species. As expected, the removal of both species traits and species random effects resulted in a model that was unable to capture any inter-specific variation observed among species, and thus had low discriminatory ability. Lastly, we found the removal of the carbon-independent hazard term ($\gamma_s$) resulted in a lower $r^2$ relative to the removal of the carbon-dependent hazard ($\alpha_s e^{-\beta_s X_{i}}$), indicating that the former hazard explained more inter-specific variation. However, when examining the removal of these hazards in terms of the model's ability to correctly distinguish survivors from those that died (AUROC), we found the opposite -- that the removal of the carbon-dependent hazard resulted in substantially lower predictive ability.

\section*{Discussion}
Our Bayesian framework coupled with cross validation revealed that the most predictive hazard model of tropical tree death was one which: 1) partitioned mortality into carbon-dependent and carbon-independent hazards; 2) used stem diameter growth rather than basal-area growth; 3) included the effects of species' maximum dbh, wood density and light demand; and 4) incorporated temporal variability. Moreover, we found that rates of tropical tree mortality varied substantially between species and that the a species' maximum dbh, wood density and light demand explained only a fraction of the overall inter-specific variation.

The findings of this study provide empirical support for dynamic vegetation models that predict tree mortality as a function of both carbon-dependent and independent hazards \cite{Pacala:1996vm, Moorcroft:2001ws, McDowell:2011dr}. We show that regardless of growth measure (our proxy for carbon budget), incorporating both hazards significantly improves model predictive skill. This is because the carbon-dependent hazard allows for deaths associated with low carbon budgets, and as a consequence, incorporates intra-specific variability attributed to carbon related stresses (e.g. competition, parasites, herbivory). By contrast, the carbon-independent hazard accounts for deaths caused by events that arise irrespective of an individual's carbon budget (e.g. windthrow, lightning strike).

Additionally, the partitioning of mortality rates into both carbon-dependent and carbon-independent effects allowed us to not only estimate their relative contribution, but also how species traits affect each. Like many other studies \cite{King:1994vg, Ruger:2011cv, Kunstler:2016km}, our analyses highlighted the importance of light competition in influencing tropical tree demographic rates. Specifically, we found that that the removal of the carbon-dependent hazard term resulted in a substantial reduction in the model's ability to correctly distinguish survivors from those that died (Table~\ref{table:1}). This suggests deficiencies in carbon budgets are a major contributor to tree death on Barro Colorado Island.

Our analyses support previous research that advocated that wood density, species maximum size and light demand important explanatory variables of inter-specific variation in tropical rainforests \cite{Poorter:2006tq, Chave:2009iy}. Specificially, we show that light demanding species are not only more susceptible to both low growth rates and growth-independent hazards, but also exhibited shallower declines in growth-dependent hazards with increasing growth rates. By contrast, both high dbh or wood density species were found to be more tolerant of growth-independent hazards, corrobrating evidence reported elsewhere \cite{Chave:2009iy, Kraft:2010kq}. However, our analyses has also supports other work \cite{vanGelder:2006exa, Kunstler:2016km} which highlights that higher wood density species are also more tolerant of low growth rates, and thus, better able to tolerate shade and competiton.

While the effects of species' wood density, maximum size and light demand are now varyingly being incorporated into mortality algorithms of many vegetation models \cite{Moorcroft:2001ws,Scheiter:2013ed}, our analysis indicate that such effects are likely to only capture a small proportion of the overall inter-specific variation (Fig. \ref{fig4}, Table~\ref{table:1}). This means that such models are likely to severely underestimate the true variability in tropical rainforest mortality rates. As a consequence, this underestimation is likely to manifest in biased estimates of carbon, water and nutrient dynamics of ecosystems \cite{Johnson-2016}.

% Daniel I find these next three paragraphs as ho humm. I'm advocating for their deletion. Instead, we could just end with the above? Thoughts? 

%Future research should focus on resolving how climate influences both carbon-dependent and carbon-independent hazards, particularly as climate-driven mortality is increasing \cite{vanMantgem:2009bi,Anderegg:2012ep,Mcdowell:2013wf}. Our analyses revealed a marginal decline in mortality at BCI from 1995 to 2010. Attributing temporal changes to particular climatic variables is challenging, however, due to low number temporal replication (n=3).

%We should also consider how the interval between censuses affects the estimation of carbon-dependent and carbon-independent hazards. Large census intervals may underestimate carbon-dependent mortality, as wide census intervals will not capture deaths due to rapid declines in growth, or events such as drought (although drought might also increase tree growth \cite{Condit2017}). Consequently, we may overestimate the relative contribution of carbon-independent hazards. 

%Here we showcase a new framework for modelling tropical tree mortality that unifies empirical evidence from within and between species studies. This framework also provides an approach for partitioning mortality rates into carbon-dependent and carbon-independent hazards. Our findings reveal that while wood density is an important trait affecting mortality rates, we are still only capturing a fraction of the overall species variability in mortality rates.

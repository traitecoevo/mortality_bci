...
A further advantage of the proposed hazard function (eq. {eq:model}) is that it allows us to estimate (at population level), the relative contributions of different mortality events, even if the causes of death for each individual tree remain unknown. 

...
Using the hazard function with highest predictive skill (i.e. $(\alpha e^{-\beta X_{i}} + \gamma)\delta_t$), we examined how model predictive accuracy was influenced by adding the effects of species' maximum dbh, light demand and wood density to $\alpha$, $\beta$ and $\gamma$. In general we found that including traits effects on these parameters decreased logarithmic loss, and thus, increased model predictive accuracy (Fig.~\ref{fig2}). Moreover, we found that incorporating the effect of all three traits on these parameters resulted in better predictive model relative to models that only incorporated one or two of the traits. However, relative to a model including species random effects on each parameter, which account for unexplained species differences, the performance gain from these traits was small, signifying that substantial inter-specific variation in mortality rates remains unexplained (Fig. \ref{fig2} Stage 6). See Figs. S3-S5 for species-level parameter estimates. 

Examining the effects of maximum dbh, light demand and wood density on $\alpha$, $\beta$ and $\gamma$ revealed that each trait influenced tree mortality in different ways (Fig. \ref{fig3}-\ref{fig4}). Specifically, we found that light demanding species were more susceptible to low growth rates (i.e. higher $\alpha$), had higher growth-independent hazards (i.e. higher $\gamma$) and exhibited shallower declines in growth-dependent hazards with increasing growth rates (i.e. lower $\beta$). By contrast, we found that wood density had significant impacts on $\alpha$ and $\gamma$, such that species with high wood density were less susceptible to low growth rates and had lower growth-independent hazards. Species' maximum dbh was found to only have a significant effect on $\gamma$, whereby species that can achieve larger dbh exhibited lower growth-independent hazards.

\begin{figure}[h!]
\includegraphics{figures/fig4.pdf}
\caption{Predicted changes in annual mortality probability with increasing growth rate. A) Estimated average curves for all 203 Barro Colorado Island species used in this study. B,C,D) represent average ($\pm$ 95\% credible intervals) curve for a high (red) and low (blue) trait values. B) wood density values 0.2 vs 0.8 (g cm$^{-3}$); C) maximum dbh values 3 vs 180 cm; D) light demand values 0.5 vs 0.8.}
\label{fig4}
\end{figure}

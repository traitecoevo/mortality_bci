...
Plants die via many causes, however, these broadly fall into two main categories \cite{Hawkes:2000ib,McDowell:2011dr}. The first are carbon-dependent hazards, where plants die because of insufficient carbon assimilation for growth and repair. The second are carbon-independent hazards, where plants die because of chance events that are unrelated to their growth rate, such as windfall or fire. An individual's total hazard at a given time point is therefore the sum of these two independent hazards. However, to date, statistical models of plant mortality have not attempted to simulataneously estimate these hazards from standard survival data, let alone examine how intra- and inter-specific factors affect them.

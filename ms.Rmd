...
## Data
We evaluate how wood density affects both the growth–mortality trade-off and growth-indepenent mortality using survival and growth data of 179 940 trees spanning 203 species from within the 50 ha tropical rainforest plot on Barro Colorado Island (BCI), Panama. Within this plot, the diameter and survival status of all free-standing woody plants with a diameter at breast height (dbh) greater or equal to 1 cm were recorded in 1982, 1990, 1995, 2000, 2005 and 2010 @BarroColoradofores:2012up. For the purpose of this study, we excluded data collected in 1982 and 1985 censuses because diameter measurements were rounded to the nearest 5 mm whereas later censuses were rounded to the nearest millimetre. We also excluded palms and ferns because they lack dbh growth @Wright:2010fl, multistemmed plants, species that did not contain a wood density estimate or contained fewer than 10 individuals and individuals that were not consistently measured at 1.3 m or that 'returned from the dead'. We also removed individuals that were deemed to contain erroneous observations using the CTFS error checker. This removed individuals that had annual dbh growth rates greater than 75 mm/yr and individuals that had excessively negative growth rates (> - 4 mm/yr). 

Because we were interested in determining how growth influenced annual mortality rates and whether the type of growth rate affected the  predictive capacity or the effects of wood density we calculated four commonly used growth rate measures: absolute dbh and basal growth, and relative dbh and basal growth. Absolute dbh growth rate was calculated as: $\frac{dbh_t - dbh_{t-1}}{\Delta{days/365.25}}$ and relative dbh growth was calculated as: $\frac{ \log_e(dbh_t) - \log_e(dbh_{t-1})}{\Delta{days/365.25}}$. Basal area for each individual in each census was calculated as: $0.25 \times \pi \times dbh_t^2$. Absolute and relative basal area growth were then calculated the same way as dbh. All growth rates were measured in metres/yr.
...
We extend a common functional form, $M= \alpha \times \exp(-\beta \times growth)$, used to examine the growth-mortality relationship [@Kobe:1997vy] to include mortality associated with growth independent processes. Specifically, we extend this negative exponential to include an asymptote parameter,$\gamma$, such that the functional form now becomes $M= \alpha \times \exp(-\beta \times growth) + \gamma$. The advantage of adding $\gamma$ is that when growth rates are high the negative exponential asymptotes at a mortality rate above zero. This allows for deaths to occur at high growth rates that would otherwise not be accounted for in the original functional form. Another advantage of this functional form is that all parameters are biologically interpretable. $\alpha$ and $\beta$ are interpreted similarly to how they have been elsewhere [kobe1997]; $\alpha$ represents the mortality rate at zero growth (and controls where the function crosses the y-axis) and $\beta$ represents the sensitivity of mortality rates to changes in growth rate (corresponding to the function's decay). We interpret $\gamma$ as the growth-independent mortality rate (corresponding to where the function asymptotes). This parameter accounts for mortality not related to growth (e.g. windfall, fire).

While this model can be used to examine mortality for individual species, it has limited capacity to provide general insights into the mechanisms affecting multiple species. One way add this capacity is to allow $\alpha$, $\beta$ and $\gamma$ to vary by traits (measures of species strategy). Furthermore, we can also include a species level random effect on these parameters that will effectively account for observation error and partition both explained and unexplained variation at multiple levels of a dataset (e.g. individuals, species, census)[@Gelman:2007te; @Kery:2010tp].

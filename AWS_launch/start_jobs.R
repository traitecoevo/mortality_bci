...
func_growth_tasks <- tasks_2_run(comparison = 'function_growth_comparison',iter = 50, 
                     path="/home/data")
res <- enqueue_bulk(func_growth_tasks, model_compiler, obj, progress_bar = TRUE)

# PHASE 2: BEST GROWTH & FUNCTIONAL FORM
species_re_tasks <- tasks_2_run(comparison = 'species_random_effects',iter = 20, 
                     path="/home/data")
res <- enqueue_bulk(species_re_tasks, model_compiler, obj, progress_bar = TRUE)

# PHASE 3:  BEST RHO COMBINATION
null_re_tasks <- tasks_2_run(comparison = 'rho_combinations',iter = 20, 
                     path="/home/data")
